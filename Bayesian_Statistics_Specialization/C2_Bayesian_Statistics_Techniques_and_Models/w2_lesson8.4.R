...
data3_extra_jags = list(y=log(warpbreaks$breaks), woolGrp=as.numeric(warpbreaks$wool), tensGrp=as.numeric(warpbreaks$tension))

params3_extra = c("mu", "sig")

mod3_extra = jags.model(textConnection(mod3_extra_string), data=data3_extra_jags, n.chains=3)
update(mod3_extra, 1e3)


(dic3_extra = dic.samples(mod3_extra, n.iter=1e3))
